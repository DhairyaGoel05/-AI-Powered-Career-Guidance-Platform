...
class CareerChatAssistant:
    ...
    def initialize_rag(self, career_data):
        ...
        # --- Changed check to google_api_key ---
        if not self.google_api_key or not career_data or not self.career_system:
            return False
        ...
        try:
            ...
            # --- Use the LLM from the career_system instead of creating a new one ---
            llm = self.career_system.llm
            ...
            st.error(f"Error initializing chat assistant: {str(e)}") # Changed to st.error for visibility
            return False
